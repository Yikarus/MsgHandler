...
def receive_image(sock):
    ...
    # print("data:", depth_data)
    return (color_data, depth_data)

def send_commands(conn):
    while True:
        command = input("Enter command (type 's' to save the image, 'exit' to quit): ")
        if command.lower() == 'exit':
            conn.sendall(command.encode())
            break
        print("saved in remote server")
        conn.sendall(command.encode())

def client_program():
    host = '192.168.0.108'  # 服务器的 IP 地址
    port = 12345  # 端口号必须与服务器相同
    
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((host, port))
    receiver_thread = threading.Thread(target=send_commands, args=(client_socket,))
    receiver_thread.daemon = True  # 设置为守护线程，以便主程序结束时子线程自动退出
    ...
    while True:
        try:
            ...
            if depth_image is not None:
                ...
                depth_image = cv2.imdecode(np.frombuffer(depth_image, dtype=np.uint8), cv2.IMREAD_COLOR)
