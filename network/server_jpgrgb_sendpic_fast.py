...
def recv_commands(conn):
    ...
    while True:
        ...
        if not data:
            break
        print("receive cmd from client:", data)
        if data.decode() == "s":
            IsNeedSave = True
...
def main():
    ...
    while True:
        ...
        receiver_thread = threading.Thread(target=recv_commands, args=(conn,))
        ...
        while True:
            try:
                ...
                if color_image is None:
                    ...
                    continue
                # At this time, rgb is jpg and depth_img is u16
                if IsNeedSave == True:
                    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
                    # jpg = cv2.imencode('.jpg', color_image)
