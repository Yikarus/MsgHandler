...
def receive_image(sock):
    ...
    print("recv depth png size :", depth_size)
    color_data = b''
    ...
    while len(depth_data) < depth_size:
        ...
        if not packet:
            return (None, None)
        depth_data += packet
    depth_raw_image = cv2.imdecode(np.frombuffer(depth_data, dtype=np.uint8), cv2.IMREAD_UNCHANGED | cv2.IMREAD_ANYDEPTH)
    print("depth_raw_image:", depth_raw_image.shape)
    depth_image = cv2.normalize(depth_raw_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
